...
public class AudioCreatorGUI : BaseCreatorGUI<AudioNode>
{
    ...
    protected override void OnDrop(AudioNode node, UnityEngine.Object[] objects)
    {
        if (objects[0] as AudioNode != null) //Drag N Drop internally in the tree, change the parent
        {
            ...
            AudioBankLink newBank = node.GetBank();
            ...
            if (UndoHelper.IsNewUndo)
            {
                UndoHelper.DoInGroup(() =>
                {
                    ...
                    UndoHelper.RegisterFullObjectHierarchyUndo(oldBank.LazyBankFetch);
                    UndoHelper.RegisterFullObjectHierarchyUndo(newBank.LazyBankFetch);    
                });
            }
            else
            {
