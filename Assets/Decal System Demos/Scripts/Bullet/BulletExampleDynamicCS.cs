...
public class BulletExampleDynamicCS : MonoBehaviour {
	...
	private void Update () {
		if (Input.GetButtonDown ("Fire1")) {
			...
			RaycastHit hitInfo;
			if (Physics.Raycast (l_Ray, out hitInfo, Mathf.Infinity)) {
				...
				if (hitInfo.rigidbody == null) {
					...
					Vector3 l_ProjectorPosition = hitInfo.point - (decalProjectorOffset * l_Ray.direction.normalized);
					...
					TerrainCollider l_TerrainCollider = hitInfo.collider as TerrainCollider;
					if (l_TerrainCollider != null) {
						...
						MeshCollider l_MeshCollider = hitInfo.collider.GetComponent <MeshCollider> ();
						MeshFilter l_MeshFilter = hitInfo.collider.GetComponent <MeshFilter> ();
						...
						if (l_MeshCollider != null || l_MeshFilter != null) {
							...
							if (l_Mesh != null) {
								...
								Matrix4x4 l_WorldToMeshMatrix = hitInfo.collider.renderer.transform.worldToLocalMatrix;
								Matrix4x4 l_MeshToWorldMatrix = hitInfo.collider.renderer.transform.localToWorldMatrix;
						...
						// Dynamic object case.
					BulletExampleDynamicObjectCS l_BulletExampleDynamicObject = hitInfo.collider.GetComponent <BulletExampleDynamicObjectCS> ();
					l_BulletExampleDynamicObject.AddDecalProjector (l_Ray, hitInfo);
				}
			}
		}
	}
