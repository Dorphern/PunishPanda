...
public class BulletLightmapExampleCS : MonoBehaviour {
	...
	private void Update () {
		if (Input.GetButtonDown ("Fire1")) {
			...
			if
				(Physics.Raycast (l_Ray, out hitInfo, Mathf.Infinity) &&
				 hitInfo.collider as TerrainCollider == null)
			{
				...
				MeshRenderer l_MeshRenderer = hitInfo.collider.GetComponent <MeshRenderer> ();
